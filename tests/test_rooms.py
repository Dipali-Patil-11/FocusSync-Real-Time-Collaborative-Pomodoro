...
def test_room_creation_and_joining():
    ...
    # 1. Create room
    ok, msg, res = service.create_room("User 1")
    assert ok is True
    code = res["room_code"]
    ...
    # 2. Join room User 2
    ok2, msg2, res2, status2 = service.join_room(code, "User 2")
    assert ok2 is True
    assert status2 == 'OK'
    ...
    ok3, msg3, res3, status3 = service.join_room(code, "User 3")
    assert ok3 is False
    assert status3 == 'FULL'
    assert "full" in msg3.lower()

    # 4. Reconnect User 1
    ok_recon, msg_recon, res_recon, status_recon = service.join_room(code, "User 1", token1)
