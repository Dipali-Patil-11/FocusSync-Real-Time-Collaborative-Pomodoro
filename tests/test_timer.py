...
def test_timer_lifecycle():
    ...
    # Start
    ok, msg, timer = timer_service.start_timer(session_code)
    assert ok is True
    assert timer.status == TimerStatus.RUNNING.value
    assert timer.target_end_time is not None

    # Pause
    ok, msg, timer = timer_service.pause_timer(session_code)
    assert ok is True
    assert timer.status == TimerStatus.PAUSED.value

    # Change Mode to Short Break
    ok, msg, timer = timer_service.change_mode(session_code, TimerMode.SHORT_BREAK.value, settings)
    assert ok is True
    ...
    # Reset
    ok, msg, timer = timer_service.reset_timer(session_code)
    assert ok is True
    assert timer.status == TimerStatus.IDLE.value
    assert timer.remaining_seconds == 300
